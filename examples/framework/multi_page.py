...
from streamlit_plugins.framework.multilit import Multilit, STPageWrapper
...
def run():
    ...
    st_demo1_page = st.Page(demo_1_page, title="Demo 1")
    st_demo2_page = st.Page(demo_2_page, title="Demo 2")

    @multilit.page(title="Home", app_type="home")
    def my_home():
        st.info('HOME')
        multilit.change_page_button()
        if st.button('Demo1', key="change-app_demo1"):
            multilit.change_page(st_demo1_page.id)
        if st.button('Demo2', key="change-app_demo2"):
            multilit.change_page(st_demo2_page.id)

    multilit.add_page(title="Demo1", page=st_demo1_page, icon=None)
    multilit.add_page(title="Demo2", page=st_demo2_page, icon=None)

    # @multi_app.addapp(title="Account", app_type="account")
    # def my_home():

    multilit.get_nav_transition()
